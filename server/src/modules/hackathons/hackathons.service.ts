...
import { addReputation, addTeamReputation, rewardTeamMembers, awardBadge } from "modules/reputation/reputation.service";
...
export const registerTeamForHackathon =  async (
    ...
    if (
      ...
      throw new AppError(
        "Team exceeds maximum allowed size",
        400
      );
    }
    ...
    const existingRegistration =
      await prisma.hackathonRegistration.findUnique({
        where: {
          hackathonId_teamId: {
            hackathonId,
            teamId,
...
export const submitProjectToHackathon =  async (
    ...
    if (
      ...
      throw new AppError(
        "Deleted project cannot be submitted",
        400
      );
    }
...
export const deleteHackathon =  async (
  ...
  export const assignJudgeToHackathon =  async (
    ...
    //
    const existingJudge =
      await prisma.hackathonJudge.findUnique({
        where: {
          hackathonId_userId: {
            hackathonId,
            userId:
  ...
  export const evaluateSubmission =  async (
    ...
    //
    const submission =
      await prisma.hackathonSubmission.findUnique({
        where: {
          id: submissionId,
        },
    ...
    if (!judge) {
      throw new AppError(
        "You are not a judge for this hackathon",
        403
      );
    }
    ...
    if (
      isTeamMember &&
      !judge.canEvaluateOwnTeam
    ) {
      throw new AppError(
        "Cannot evaluate your own team",
        400
      );
    }
    ...
    const existingEvaluation =
      await prisma.hackathonEvaluation.findUnique({
        where: {
    ...
    //
    const evaluation =
      await prisma.hackathonEvaluation.create({
        data: {
          hackathonId:
            submission.hackathonId,
    ...
    //
    const evaluations =
      await prisma.hackathonEvaluation.findMany({
        where: {
          submissionId,
        },
      });
...
export const getHackathonLeaderboard =  async (
    hackathonId: string
  ) => {
    ...
    //
    const submissions =
      await prisma.hackathonSubmission.findMany({
        where: {
          hackathonId,
