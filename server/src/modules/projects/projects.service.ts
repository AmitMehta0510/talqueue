...
import { addReputation, addTeamReputation} from "modules/reputation/reputation.service";
...
import { calculateProjectVerificationScore, calculateProjectEngineeringScore } from "modules/projects/project-auth.service";
...
export const getProjectById =  async (
    ...
    if (
      engineeringScore >= 200
    ) {
      trustLevel = "ELITE";
    } else if (
      engineeringScore >= 120
    ) {
      trustLevel = "HIGH";
    } else if (
      engineeringScore >= 60
...
export const requestToJoinProject =  async (
    ...
    const existingPendingRequest =
      await prisma.projectJoinRequest.findFirst({
        where: {
          projectId,
          userId,
          status: "PENDING",
    ...
    if (recentRejectedRequest) {
      ...
      const diff =
        Date.now() -
        new Date(
          recentRejectedRequest.createdAt
        ).getTime();

      const cooldownMs =
        PROJECT_JOIN_REQUEST_COOLDOWN_HOURS *
        60 *
        60 *
        1000;
...
createNotification({
  ...
  message:
    `${requester?.profile?.fullName || requester?.username} requested to join "${project.title}"`,
...
export const reviewJoinRequest =  async (
    ...
    if (
      request.project.ownerId !== ownerId
    ) {
      throw new AppError(
        "Unauthorized",
        403
...
createNotification({
  ...
  groupKey:
    `project-review-${request.projectId}`,
}).catch((error) => {
  console.error(
    "Notification Error:",
    error
...
export const inviteUserToProject =  async (
    ...
    const existingInvite =
      await prisma.projectInvite.findFirst({
        where: {
          projectId,
...
createNotification({
  ...
  message:
    `${owner?.profile?.fullName || owner?.username} invited you to join "${invite.project.title}"`,
...
createNotification({
  ...
  groupKey:
    `project-invite-review-${invite.projectId}`,
}).catch((error) => {
  console.error(
    "Notification Error:",
    error
    ...
    await Promise.all(
      project.members.map(
        async (member) => {
          ...
          await calculateUserAffinity(ownerId,
  member.userId);
        }
      )
    );
...
export const syncGithubProject =  async (
    userId: string,
    projectId: string
  ) => {
