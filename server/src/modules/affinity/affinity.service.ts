...
export const calculateUserAffinity = async (
  ...
  const targetSkillNames = targetSkills.map((s) => s.skill.name.toLowerCase());
  ...
  recruiterScore += profileViews * 3;

  score += profileViews * 3;
  ...
  return prisma.userAffinity.upsert({
    ...
    update: {
      ...
      interactionCount: totalMessages + interactions,
...
export const rebuildUserAffinities = async (userId: string) => {
  ...
  //
  await Promise.all(
    users.map((user) => calculateUserAffinity(userId, user.id)),
  );
